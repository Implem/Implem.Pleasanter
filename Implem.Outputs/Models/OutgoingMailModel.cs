    ...
    public class OutgoingMailModel : BaseModel
    {
        ...
        public string DeleteComment()
        {
            ...
            if (Rds.ExecuteScalar_int(
                transactional: true,
                statements: Rds.UpdateOutgoingMails(
                    verUp: VerUp,
                    where: Rds.OutgoingMailsWhereDefault(this)
                        .UpdatedTime(Forms.DateTime("OutgoingMails_Timestamp")),
                    param: Rds.OutgoingMailsParamDefault(this),
                    countRecord: true)) == 0)
            {
                return ResponseConflicts();
            }
            Get();
            return ResponseByUpdate(new OutgoingMailsResponseCollection(this))
            ...
            OutgoingMailModel outgoingMailModel, Message message = null, bool pushState = true)
        {
            ...
            return new OutgoingMailsResponseCollection(this)
                ...
                .Message(message)
                .PushState(
                    Navigations.Get("OutgoingMails", OutgoingMailId.ToString(), "Reload"),
    ...
    public static class OutgoingMailsUtility
    {
        ...
        private static string Grid(SiteSettings siteSettings, Permissions.Types permissionType)
        {
            ...
            return new ResponseCollection()
                .Html("#DataViewContainer", outgoingMailCollection.Count > 0
                    ? Html.Builder().Grid(
                        siteSettings: siteSettings,
                        outgoingMailCollection: outgoingMailCollection,
                        permissionType: permissionType,
            ...
            return hb.FieldSet(id: "FieldSetGeneral", action: () =>
            {
                siteSettings.ColumnCollection
                    ...
                    .ForEach(column =>
                    {
                        switch (column.ColumnName)
                        {
                            case "ReferenceType": hb.Field(siteSettings, column, outgoingMailModel.ReferenceType.ToControl(column), column.ColumnPermissionType(permissionType)); break;
                            case "ReferenceId": hb.Field(siteSettings, column, outgoingMailModel.ReferenceId.ToControl(column), column.ColumnPermissionType(permissionType)); break;
                            case "OutgoingMailId": hb.Field(siteSettings, column, outgoingMailModel.OutgoingMailId.ToControl(column), column.ColumnPermissionType(permissionType)); break;
                            case "Ver": hb.Field(siteSettings, column, outgoingMailModel.Ver.ToControl(column), column.ColumnPermissionType(permissionType)); break;
                            case "To": hb.Field(siteSettings, column, outgoingMailModel.To.ToControl(column), column.ColumnPermissionType(permissionType)); break;
                            case "Cc": hb.Field(siteSettings, column, outgoingMailModel.Cc.ToControl(column), column.ColumnPermissionType(permissionType)); break;
                            case "Bcc": hb.Field(siteSettings, column, outgoingMailModel.Bcc.ToControl(column), column.ColumnPermissionType(permissionType)); break;
                            case "Title": hb.Field(siteSettings, column, outgoingMailModel.Title.ToControl(column), column.ColumnPermissionType(permissionType)); break;
                            case "Body": hb.Field(siteSettings, column, outgoingMailModel.Body.ToControl(column), column.ColumnPermissionType(permissionType)); break;
                            case "SentTime": hb.Field(siteSettings, column, outgoingMailModel.SentTime.ToControl(column), column.ColumnPermissionType(permissionType)); break;
                            case "DestinationSearchRange": hb.Field(siteSettings, column, outgoingMailModel.DestinationSearchRange.ToControl(column), column.ColumnPermissionType(permissionType)); break;
                            case "DestinationSearchText": hb.Field(siteSettings, column, outgoingMailModel.DestinationSearchText.ToControl(column), column.ColumnPermissionType(permissionType)); break;
                        }
                    });
