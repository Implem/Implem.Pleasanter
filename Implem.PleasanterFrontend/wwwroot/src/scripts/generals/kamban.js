$p.setKamban = function (suffix) {
    ...
    if (suffix) {
        suffixElArr = $('#MainForm')
            .find('div[id="KambanBody_' + suffix + '"].kambanbody')
            .get();
    }
            ...
            .each(function () {
                ...
                $(this).on('touchstart', function (e) {
                    kambanSuffix = $(this)
                        .parents('[id^=Kamban]')
                        .attr('id')
                        .substring($(this).parents('[id^=Kamban]').attr('id').indexOf('_'));
                    kambanSuffix = kambanSuffix.indexOf('_') === -1 ? '' : kambanSuffix;
                ...
                $(this).on('touchend', function () {
                    ...
                    $('#KambanBody' + kambanSuffix + ' .kamban-container').each(function () {
                        ...
                        if (
                            ...
                            var data = $p.getData($('.main-form'));
                            var tableNamePrefix =
                                $('#KambanReferenceType' + kambanSuffix).val() + '_';
                            var dataX = $(this).attr('data-x');
                            var dataY = $(this).attr('data-y');
                            data['KambanId'] = id;
                            if (dataX !== undefined) {
                                data[tableNamePrefix + $('#KambanGroupByX' + kambanSuffix).val()] =
                                    dataX;
                            }
                            if (dataY !== undefined) {
                                data[tableNamePrefix + $('#KambanGroupByY' + kambanSuffix).val()] =
                                    dataY;
                            }
                            if (kambanSuffix !== '') {
                                data.SiteId = siteId;
                            ...
                            $p.set(
                                $('#KambanSuffix' + kambanSuffix),
                                $('#KambanSuffix' + kambanSuffix).val()
                            );
                            $p.send($('#KambanBody' + kambanSuffix));
