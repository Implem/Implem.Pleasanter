...
{
    [Collection(nameof(DeptsPhysicalDelete))]
    public class DeptsPhysicalDelete
    {
        static string DeletedDept = "組織15（物理削除）";
        public DeptsPhysicalDelete()
        {
        ...
        {
            var deletedDeptId = Initializer.Depts.GetDeptIdByName(DeletedDept);
            var testParts = new List<TestPart>()
            {
                new TestPart(
                    queryStrings: QueryStringsUtilities.Get(
                        new KeyValue("GridCheckedItems", $"{deletedDeptId}"),
                        new KeyValue("EditOnGrid", ""),
                        new KeyValue("NewRowId", "")),
                    baseTests: BaseData.Tests(
                        JsonData.Value(
                            method: "Message",
                            value: "{\"Id\":\"PhysicalBulkDeletedFromRecycleBin\",\"Text\":\"ごみ箱から 1 件 削除しました。\",\"Css\":\"alert-success\"}")),
        ...
        private static string Results(
            Context context)
        {
            return DeptUtilities.PhysicalBulkDelete(
                context: context,
