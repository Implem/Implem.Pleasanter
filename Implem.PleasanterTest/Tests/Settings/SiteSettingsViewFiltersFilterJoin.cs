...
{
    [Collection(nameof(SiteSettingsViewFiltersFilterJoin))]
    public class SiteSettingsViewFiltersFilterJoin
    {
            ...
            var testParts = new List<TestPart>()
            {
                new TestPart(
                    ...
                    forms: FormsUtilities.Get(
                        new KeyValue("ControlId", "ViewFiltersFilterJoin"),
                        new KeyValue("ViewFiltersFilterJoin", "")),  //""の指定で、そのテーブル自身がもつカラムを選択肢一覧に設定する動作となります。
                    baseTests: BaseData.Tests(
                        JsonData.ExistsOne(
                            method: "Html",
                            target: "#ViewFiltersFilterSourceColumns"),
                        JsonData.ExistsOne(
                            method: "SetMemory",
                            target: "formChanged"),
