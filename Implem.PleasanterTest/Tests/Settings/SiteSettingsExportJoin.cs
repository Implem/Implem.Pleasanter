...
{
    [Collection(nameof(SiteSettingsExportJoin))]
    public class SiteSettingsExportJoin
    {
            ...
            var testParts = new List<TestPart>()
            {
                new TestPart(
                    ...
                    forms: FormsUtilities.Get(
                        new KeyValue("ControlId", "ExportJoin"),
                        new KeyValue("ExportJoin", "")),  //""の指定で、そのテーブル自身がもつカラムを選択肢一覧に設定する動作となります。
                    baseTests: BaseData.Tests(
                        JsonData.ExistsOne(
                            method: "Html",
                            target: "#ExportSourceColumns"),
                        JsonData.ExistsOne(
                            method: "SetFormData",
                            target: "ExportSourceColumns"),
                        JsonData.ExistsOne(
