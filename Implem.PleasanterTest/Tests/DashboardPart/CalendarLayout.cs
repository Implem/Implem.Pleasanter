...
{
    [Collection(nameof(CalendarLayout))]
    public class CalendarLayout
    {
        ...
        public static IEnumerable<object[]> GetData()
        {
            ...
            var testParts = new List<TestPart>()
            {
                ...
                // テスト
                //「タイトルを表示する：true」「参照先が期限付きテーブル」
                new TestPart(
                    baseTests: BaseData.Tests(
                        // ダッシュボードパーツの確認
                        HtmlData.TextContains(selector: "#DashboardPartLayouts", value: "DashboardPart_8"),
                // タイトルが表示されていることの確認
                        HtmlData.TextContains(selector: "#DashboardPartLayouts", value: "CalendarLayout(「タイトルを表示する：true」「参照先が期限付きテーブル」)"))),
                //「参照先が記録テーブル」
                new TestPart(
                    baseTests: BaseData.Tests(
                        // ダッシュボードパーツの確認
                        HtmlData.TextContains(selector: "#DashboardPartLayouts", value: "DashboardPart_9"),
                        // Urlが指定されていないことの確認
                        HtmlData.TextContains(selector: "#DashboardPartLayouts", value: "FullCalendar_9"))),
                //「参照先がそれ以外(未入力)」
                new TestPart(
                    baseTests: BaseData.Tests(
                        // ダッシュボードパーツの確認
                        HtmlData.TextContains(selector: "#DashboardPartLayouts", value: "DashboardPart_10")))
            };
