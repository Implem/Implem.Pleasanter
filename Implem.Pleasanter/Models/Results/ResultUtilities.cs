...
namespace Implem.Pleasanter.Models
{
    public static class ResultUtilities
    {
        ...
        {
            var gridSqlColumn = Rds.ResultsColumn()
                .SiteId()
                .ResultId()
                .Creator()
                .Updator();
            siteSettings.GridColumnCollection(withTitle: true).ForEach(column =>
                Rds.ResultsColumn(gridSqlColumn, column.ColumnName));
        ...
        {
            var resultModel = new ResultModel(siteSettings, permissionType, 0, setByForm: true);
            var invalid = ValidateBeforeCreate(siteSettings, permissionType, resultModel);
            switch (invalid)
            {
            ...
            else
            {
                ...
                return ResponseByUpdate(resultModel, responseCollection)
        ...
        {
            var resultModel = new ResultModel(
                siteSettings, permissionType, resultId, setByForm: true);
            var invalid = ValidateBeforeUpdate(siteSettings, permissionType, resultModel);
            switch (invalid)
            {
                case Error.Types.None: break;
        ...
        private static ResponseCollection EditorResponse(ResultModel resultModel, Message message = null)
        {
