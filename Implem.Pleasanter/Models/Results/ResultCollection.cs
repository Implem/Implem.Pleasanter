...
namespace Implem.Pleasanter.Models
{
    public class ResultCollection : List<ResultModel>
    {
        ...
        public ResultCollection(SiteSettings ss, EnumerableRowCollection<DataRow> dataRows)
        {
            Set(ss, dataRows);
        }

        private ResultCollection Set(SiteSettings ss, EnumerableRowCollection<DataRow> dataRows)
        {
            if (dataRows.Any())
            {
            ...
            IEnumerable<Aggregation> aggregations = null)
        {
            ...
            {
                statements.AddRange(Rds.ResultsAggregations(
                    aggregations: aggregations,
                    join: join ??  Rds.ResultsJoinDefault(),
                    where: where));
            }
