...
namespace Implem.Pleasanter.Models
{
    public static class TenantUtilities
    {
        ...
        private static string ViewModeTemplate(
            ...
            Action viewModeBody)
        {
            var invalid = TenantValidators.OnEntry(
                context: context,
                ss: ss);
            switch (invalid)
            {
        ...
        {
            var sqlColumnCollection = Rds.TenantsColumn();
            new List<string> { "TenantId", "Creator", "Updator" }
                ...
                .Concat(ss.TitleColumns)
                    .Distinct().ForEach(column =>
                        sqlColumnCollection.TenantsColumn(column));
