...
namespace Implem.Pleasanter.Models
{
    public class StatusCollection : List<StatusModel>
    {
        ...
        public StatusCollection(EnumerableRowCollection<DataRow> dataRows)
        {
            Set(dataRows);
        }

        private StatusCollection Set(EnumerableRowCollection<DataRow> dataRows)
        {
            if (dataRows.Any())
            {
                foreach (DataRow dataRow in dataRows)
                {
                    Add(new StatusModel(dataRow));
                }
            ...
            IEnumerable<Aggregation> aggregationCollection = null)
        {
            ...
            {
                statements.AddRange(Rds.StatusesAggregations(aggregationCollection, where));
            }
        ...
        private EnumerableRowCollection<DataRow> Get(string commandText, SqlParamCollection param = null)
        {
            return Rds.ExecuteTable(
                transactional: false,
                statements: Rds.StatusesStatement(
