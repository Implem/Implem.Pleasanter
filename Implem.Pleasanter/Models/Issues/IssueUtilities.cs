...
namespace Implem.Pleasanter.Models
{
    public static class IssueUtilities
    {
        ...
        private static HtmlBuilder Fields(
            ...
            int tabIndex = 0)
        {
            var sectionGroups = new List<(Section Section, List<string> Columns, List<(Section Section, List<string> Columns)> Children)>();
            ss
                .GetEditorColumns(
                    context: context,
                    tab: tab,
                ...
                ?.ForEach(column =>
                {
                    ...
                    if (section != null)
                    {
                        ...
                        if (current.Level == 2
                            && sectionGroups.Any()
                            && sectionGroups.Last().Section != null)
                        {
                            sectionGroups.Last().Children.Add((current, new List<string>()));
                        }
                        else
                        {
                            sectionGroups.Add((
                                current,
                                new List<string>(),
                                new List<(Section Section, List<string> Columns)>()));
                        }
                    }
                    else
                    {
                        ...
                        var group = sectionGroups.Last();
                        (group.Children.Any()
                            ? group.Children.Last().Columns
                            : group.Columns)
            ...
            sectionGroups.ForEach(group =>
            {
                ...
                else if (group.Section.Hide != true)
                {
                    hb.Div(
                        id: $"SectionFields{group.Section.Id}Container",
                        css: "section-fields-container",
                        action: () => hb
                            .SectionHeader(section: group.Section)
                            .Div(
                                id: $"SectionFields{group.Section.Id}",
                                css: group.Section.SectionFieldsCss(),
                                action: () =>
                                {
                                    hb.Fields(
                                        context: context,
                                        ...
                                        tabIndex: tabIndex);
                                    group.Children
                                        .Where(child => child.Section.Hide != true)
                                        .ForEach(child => hb.Div(
                                            id: $"SectionFields{child.Section.Id}Container",
                                            css: "section-fields-container",
                                            action: () => hb
                                                .SectionHeader(section: child.Section)
                                                .Div(
                                                    id: $"SectionFields{child.Section.Id}",
                                                    css: child.Section.SectionFieldsCss(),
                                                    action: () => hb.Fields(
                                                        context: context,
                                                        ss: ss,
                                                        id: id,
                                                        columnNames: child.Columns,
                                                        dataSet: dataSet,
                                                        links: links,
                                                        issueModel: issueModel,
                                                        preview: preview,
        ...
        public static string Update(Context context, SiteSettings ss, long issueId, string previousTitle)
        {
            ...
            switch (errorData.Type)
            {
                case Error.Types.None:
                    ...
                    var afterUpdateHidden = issueModel.ServerScriptModelRow?.Hidden
            ...
            issueCollection.ForEach(issueModel =>
                Implem.Pleasanter.Libraries.AiConnect.AiConnectUtilities.Sync(
                    ...
                    replacedDisplayValues: value => issueModel.ReplacedDisplayValues(
                        ...
                        ss: ss,
                        value: value),
                    operation: formDataSet.Any(o => o.Id == issueModel.IssueId)
