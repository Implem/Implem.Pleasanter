...
namespace Implem.Pleasanter.Models
{
    ...
    public class IssueModel : BaseItemModel
    {
        ...
        public bool StartTime_InitialValue()
        {
            return StartTime == 0.ToDateTime();
        }

        public bool CompletionTime_InitialValue()
        {
            return CompletionTime.Value == 0.ToDateTime();
        }

        public bool WorkValue_InitialValue()
        {
            return WorkValue.Value == 0;
        }

        public bool ProgressRate_InitialValue()
        {
        ...
        private SqlStatement CopyToStatement(SqlWhereCollection where, Sqls.TableTypes tableType)
        {
            ...
            if (!StartTime_InitialValue())
            {
                column.StartTime(function: Sqls.Functions.SingleColumn);
                param.StartTime();
            }
            if (!WorkValue_InitialValue())
            {
                column.WorkValue(function: Sqls.Functions.SingleColumn);
                param.WorkValue();
            }
            if (!ProgressRate_InitialValue())
            {
                column.ProgressRate(function: Sqls.Functions.SingleColumn);
                param.ProgressRate();
            }
        ...
        public Error.Types Delete(SiteSettings ss, bool notice = false)
        {
            ...
            statements.AddRange(new List<SqlStatement>
            {
                ...
                Rds.PhysicalDeleteIssues(where: where)
            });
