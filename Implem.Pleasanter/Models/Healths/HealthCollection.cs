...
namespace Implem.Pleasanter.Models
{
    public class HealthCollection : List<HealthModel>
    {
        ...
        private HealthCollection Set(EnumerableRowCollection<DataRow> dataRows)
        {
            if (dataRows.Any())
            {
                foreach (DataRow dataRow in dataRows)
                {
                    Add(new HealthModel(dataRow));
                }
            ...
            IEnumerable<Aggregation> aggregationCollection = null)
        {
            ...
            {
                statements.AddRange(Rds.HealthsAggregations(aggregationCollection, where));
            }
        ...
        private EnumerableRowCollection<DataRow> Get(string commandText, SqlParamCollection param = null)
        {
            return Rds.ExecuteTable(
                transactional: false,
                statements: Rds.HealthsStatement(
