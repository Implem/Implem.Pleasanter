...
namespace Implem.Pleasanter.Models
{
    public class TemplateCollection : List<TemplateModel>
    {
        ...
        public TemplateCollection(EnumerableRowCollection<DataRow> dataRows)
        {
            Set(dataRows);
        }

        private TemplateCollection Set(EnumerableRowCollection<DataRow> dataRows)
        {
            if (dataRows.Any())
            {
                foreach (DataRow dataRow in dataRows)
                {
                    Add(new TemplateModel(dataRow));
                }
            ...
            IEnumerable<Aggregation> aggregationCollection = null)
        {
            ...
            {
                statements.AddRange(Rds.TemplatesAggregations(aggregationCollection, where));
            }
        ...
        private EnumerableRowCollection<DataRow> Get(string commandText, SqlParamCollection param = null)
        {
            return Rds.ExecuteTable(
                transactional: false,
                statements: Rds.TemplatesStatement(
